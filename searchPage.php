...
<!-- sidebar menu -->
<?php include('assets/includes/sidebarMenu/tabSearch.php'); ?>
    <br><br><br>
</aside>

<!-- main content area -->
<div class="main-container float-right overflow-hidden">
    <div class="wrapper">
        <div class="numOfResults">
            <?php
                if(isset($_GET['idArray'])){
                    $numOfResults = count($_GET['idArray']);
                    if($numOfResults == 0){
                        echo "<div class='alert alert-secondary mx-4 mt-3'>
                                    No Result Found !!!
                              </div>";
                    }elseif($numOfResults == 1){
                        echo "<div class='alert alert-info mx-4 mt-3'>
                                    1 Result Found.
                              </div>";
                    }else{
                        echo "<div class='alert alert-info mx-4 mt-3'>
        ...
        <div class="card-columns px-3">
            <?php
                if(isset($_GET['idArray'])){       //when search button is clicked or "see more results" is clicked
                    $searchResultsProductIdsArray = $_GET['idArray'];

                    $currentUser = $_SESSION['userLoggedIn'];           //current user
                    $collegeName = $_SESSION['currentUserCollege'];     //current user's college

                    foreach($searchResultsProductIdsArray as $searchResultsProductId){
                        //sql query to get the products
                        $query = "SELECT * FROM products WHERE id = '$searchResultsProductId' ";
                ...
                }elseif(isset($_GET['productId'])){     //when a search result is clicked********************
                    $productId = $_GET['productId'];

                    $currentUser = $_SESSION['userLoggedIn'];           //current user
                    $collegeName = $_SESSION['currentUserCollege'];     //current user's college

                    //sql query to get the corresponding products
                ...
                }else{      //when the search page is accessed directly*************************************
                    $showMsg = 1;       //show a message
                }
            ?>
        </div>
        <div class="searchResultsMessage">
           <?php
                if(isset($showMsg)){
                    echo "<div class='alert alert-secondary mx-4 mt-3'>No Results Found !!!</div>";
                }
           ?> 
        </div>    
    </div>
